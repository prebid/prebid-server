...
| `providers[].identity_url` or `providers[].context_url` | At least one is required per provider. The module POSTs to the value verbatim, so the full path matters: a bare-origin `https://tmp.example.com` will 404 — always include `/identity` and `/context`. When both are set they MUST derive to the same base URL (identity path minus `/identity`, context path minus `/context`), because TMP signatures bind to the provider's single registered base endpoint per `provider-registration.json`; a mismatch is rejected at Builder startup. |
