...
type cacheEntry struct {
	...
	timestamp                               time.Time
	hashedConfig                            hash
	ruleSetsForProcessedAuctionRequestStage []cacheRuleSet[hs.ProcessedAuctionRequestPayload, hs.HookResult[hs.ProcessedAuctionRequestPayload]]
}
...
func createCacheRuleSet(cfg *config.RuleSet) (cacheRuleSet[hs.ProcessedAuctionRequestPayload, hs.HookResult[hs.ProcessedAuctionRequestPayload]], error) {
	if cfg == nil {
		return cacheRuleSet[hs.ProcessedAuctionRequestPayload, hs.HookResult[hs.ProcessedAuctionRequestPayload]]{}, errors.New("no rules engine configuration provided")
	}

	crs := cacheRuleSet[hs.ProcessedAuctionRequestPayload, hs.HookResult[hs.ProcessedAuctionRequestPayload]]{
		name:        cfg.Name,
		modelGroups: []cacheModelGroup[hs.ProcessedAuctionRequestPayload, hs.HookResult[hs.ProcessedAuctionRequestPayload]]{},
	}

	for _, modelGroup := range cfg.ModelGroups {
		tree, err := rules.NewTree[hs.ProcessedAuctionRequestPayload, hs.HookResult[hs.ProcessedAuctionRequestPayload]](
			&treeBuilder[hs.ProcessedAuctionRequestPayload, hs.HookResult[hs.ProcessedAuctionRequestPayload]]{
				Config:            modelGroup,
		...
		cmg := cacheModelGroup[hs.ProcessedAuctionRequestPayload, hs.HookResult[hs.ProcessedAuctionRequestPayload]]{
