...
func (me *Metrics) RecordAdapterPanic(labels AdapterLabels) {
	...
	if !ok {
		logger.Error("Trying to run adapter metrics on %s: adapter metrics not found", adapterStr)
		return
	}
...
func (me *Metrics) RecordAdapterRequest(labels AdapterLabels) {
	...
	default:
		logger.Warn("No go-metrics logged for AdapterBids value: %s", labels.AdapterBids)
	}
...
func (me *Metrics) RecordAdapterConnections(adapterName openrtb_ext.BidderName,
	...
	am, ok := me.AdapterMetrics[lowerCaseAdapterName]
	if !ok {
		logger.Error("Trying to log adapter connection metrics for %s: adapter not found", string(adapterName))
		return
	}
...
func (me *Metrics) RecordAdapterBidReceived(labels AdapterLabels, bidType openrtb_ext.BidType, hasAdm bool) {
	...
	am, ok := me.AdapterMetrics[lowerCaseAdapterName]
	if !ok {
		logger.Error("Trying to run adapter bid metrics on %s: adapter metrics not found", adapterStr)
		return
	}
	...
	if metricsForType, ok := am.MarkupMetrics[bidType]; ok {
		...
		logger.Error("bid/adm metrics map entry does not exist for type %s. This is a bug, and should be reported.", bidType)
	}
}
...
func (me *Metrics) RecordAdapterPrice(labels AdapterLabels, cpm float64) {
	...
	am, ok := me.AdapterMetrics[lowercaseAdapter]
	if !ok {
		logger.Error("Trying to run adapter price metrics on %s: adapter metrics not found", adapterStr)
		return
	}
	// Adapter metrics
...
func (me *Metrics) RecordAdapterTime(labels AdapterLabels, length time.Duration) {
	...
	am, ok := me.AdapterMetrics[lowercaseAdapter]
	if !ok {
		logger.Error("Trying to run adapter latency metrics on %s: adapter metrics not found", string(labels.Adapter))
		return
	}
	// Adapter metrics
...
func (me *Metrics) RecordAdapterBuyerUIDScrubbed(adapterName openrtb_ext.BidderName) {
	...
	if !ok {
		logger.Error("Trying to log adapter buyeruid scrubbed metric for %s: adapter not found", adapterStr)
		return
	}
...
func (me *Metrics) RecordAdapterGDPRRequestBlocked(adapterName openrtb_ext.BidderName) {
	...
	am, ok := me.AdapterMetrics[strings.ToLower(adapterStr)]
	if !ok {
		logger.Error("Trying to log adapter GDPR request blocked metric for %s: adapter not found", adapterStr)
		return
	}
...
func (me *Metrics) RecordAdapterThrottled(adapterName openrtb_ext.BidderName) {
	...
	am, ok := me.AdapterMetrics[strings.ToLower(adapterStr)]
	if !ok {
		logger.Error("Trying to log adapter throttled metric for %s: adapter not found", adapterStr)
		return
	}
