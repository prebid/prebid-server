	...
	*openrtb_ext.Fledge,
	...
	for _, bidder := range bidderRequests {
		...
		bidderRunner := e.recoverSafely(bidderRequests, func(bidderRequest BidderRequest, conversions currency.Conversions) {
			...
			reqInfo.PbsEntryPointStartTime = pbsRequestStartTime
...
func (e *exchange) recordResponsePreparationMetrics(ae map[openrtb_ext.BidderName]*seatResponseExtra, requestType metrics.RequestType, now time.Time) {
	labels := metrics.OverheadLabels{
		OverheadType: metrics.NonErrorPostBidderResponseOverhead,
	}

	for _, resp := range ae {
		e.me.RecordOverheadTime(labels, resp.SeatBidsPreparationStartTime.Sub(now))
	}
}
