...
func TestAllValidBids(t *testing.T) {
	...
	seatBid, errs := bidder.requestBid(context.Background(), bidderReq, 1.0, currency.NewConstantRates(), &adapters.ExtraRequestInfo{}, nil, bidRequestOptions{accountDebugAllowed: true, headerDebugAllowed: false, addCallSignHeader: false})
...
func TestCurrencyBids(t *testing.T) {
	...
	for _, tc := range currencyTestCases {
		...
		seatBid, errs := bidder.requestBid(context.Background(), bidderRequest, 1.0, currency.NewConstantRates(), &adapters.ExtraRequestInfo{}, nil, bidRequestOptions{accountDebugAllowed: true, headerDebugAllowed: false, addCallSignHeader: false})
