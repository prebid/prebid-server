...
func TestAllValidBids(t *testing.T) {
	...
	seatBids, errs := bidder.requestBid(context.Background(), bidderReq, currency.NewConstantRates(), &adapters.ExtraRequestInfo{}, &adscert.NilSigner{}, bidReqOptions, nil)
...
func TestCurrencyBids(t *testing.T) {
	...
	for _, tc := range currencyTestCases {
		...
		seatBids, errs := bidder.requestBid(context.Background(), bidderRequest, currency.NewConstantRates(), &adapters.ExtraRequestInfo{}, &adscert.NilSigner{}, bidReqOptions, nil)
		assert.Len(t, seatBids, 1)
