...
import (
	...
	"github.com/prebid/prebid-server/adapters/playwire"
...
func newAdapterMap(client *http.Client, cfg *config.Configuration, infos adapters.BidderInfos) map[openrtb_ext.BidderName]adaptedBidder {
	ortbBidders := map[openrtb_ext.BidderName]adapters.Bidder{
		...
		openrtb_ext.BidderEngageBDROrtb:        playwire_ortb.NewOrtbBidder(cfg.Adapters[string(openrtb_ext.BidderEngageBDROrtb)].Endpoint),
		...
		openrtb_ext.BidderPlaywire:       playwire.NewPlaywireBidder(cfg.Adapters[string(openrtb_ext.BidderPlaywire)].Endpoint),
