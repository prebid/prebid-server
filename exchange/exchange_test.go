...
func TestExtractRequestPrivacyGDPR(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			&openrtb2.BidRequest{
				Regs: &openrtb2.Regs{GDPR: &SignalYes, Ext: json.RawMessage(`{"gdpr":1}`)},
				User: &openrtb2.User{Ext: json.RawMessage(`{"consent":"CPuKGCPPuKGCPNEAAAENCZCAAAAAAAAAAAAAAAAAAAAA"}`)},
			},
			config.Account{},
			gdpr.NewTCF2Config(config.TCF2{Enabled: true}, config.AccountGDPR{}),
			metrics.Labels{},
		...
		},
		{
			...
			&openrtb2.BidRequest{
				Regs: &openrtb2.Regs{GDPR: &SignalNo, Ext: json.RawMessage(`{"gdpr":0}`)},
				User: &openrtb2.User{Ext: json.RawMessage(`{"consent":"CPuKGCPPuKGCPNEAAAENCZCAAAAAAAAAAAAAAAAAAAAA"}`)},
			},
			config.Account{},
			gdpr.NewTCF2Config(config.TCF2{Enabled: true}, config.AccountGDPR{}),
		...
		{
			"Request with invalid user ext, default no, signal Yes",
			&openrtb2.BidRequest{
				Regs: &openrtb2.Regs{GDPR: &SignalYes, Ext: json.RawMessage(`{"gdpr":1}`)},
				User: &openrtb2.User{Ext: json.RawMessage(`{"CPuKGCPPuKGCPNEAAAENCZCAAAAAAAAAAAAAAAAAAAAA"}`)},
			},
			config.Account{},
			gdpr.NewTCF2Config(config.TCF2{Enabled: true}, config.AccountGDPR{}),
			metrics.Labels{},
		...
		},
		{
			...
			&openrtb2.BidRequest{
				Regs: &openrtb2.Regs{GDPR: &SignalYes, Ext: json.RawMessage(`{"gdpr1"}`)},
				User: &openrtb2.User{Ext: json.RawMessage(`{"consent":"CPuKGCPPuKGCPNEAAAENCZCAAAAAAAAAAAAAAAAAAAAA"}`)},
			},
			config.Account{},
			gdpr.NewTCF2Config(config.TCF2{Enabled: true}, config.AccountGDPR{}),
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if req.User != nil && len(req.User.Ext) > 0 {
				userExt := make(map[string]any)
				_ = json.Unmarshal(req.User.Ext, &userExt)
				if val, ok := userExt["consent"]; ok && test.requestPrivacy != nil {
					consent, _ := val.(string)
					test.requestPrivacy.ParsedConsent, _ = vendorconsent.ParseString(consent)
				}
			}
