...
func runSpec(t *testing.T, filename string, spec *exchangeSpec) {
	...
	ex := newExchangeForTests(t, filename, spec, aliases, privacyConfig, bidIdGenerator)
...
func newExchangeForTests(t *testing.T, filename string, exSpec *exchangeSpec, aliases map[string]string, privacyConfig config.Privacy, bidIDGenerator BidIDGenerator) Exchange {
	...
	for _, bidderName := range openrtb_ext.CoreBidderNames() {
		if spec, ok := exSpec.OutgoingRequests[string(bidderName)]; ok {
			...
			bidderInfos[string(bidderName)] = config.BidderInfo{ModifyingVastXmlAllowed: spec.ModifyingVastXmlAllowed, OpenRTB: &config.OpenRTBInfo{Version: exSpec.ORTBVersion}}
		} else {
...
type exchangeSpec struct {
	...
	ORTBVersion                string                 `json:"ortbversion,omitempty"`
}
