...
func NewDatabaseEventProducer(cfg DatabaseEventProducerConfig) (eventProducer *DatabaseEventProducer) {
	if cfg.Provider == nil {
		logger.Log.Fatalf("The Database Stored %s Loader needs a database connection to work.", cfg.RequestType)
	}
...
func (e *DatabaseEventProducer) fetchAll() (fetchErr error) {
	...
	if err != nil {
		logger.Log.Warningf("Failed to fetch all Stored %s data from the DB: %v", e.cfg.RequestType, err)
	...
	defer func() {
		if err := rows.Close(); err != nil {
			logger.Log.Warningf("Failed to close the Stored %s DB connection: %v", e.cfg.RequestType, err)
			e.recordError(metrics.StoredDataErrorUndefined)
			fetchErr = err
		}
	}()
	if err := e.sendEvents(rows); err != nil {
		logger.Log.Warningf("Failed to load all Stored %s data from the DB: %v", e.cfg.RequestType, err)
		e.recordError(metrics.StoredDataErrorUndefined)
		return err
	}
...
func (e *DatabaseEventProducer) fetchDelta() (fetchErr error) {
	...
	if err != nil {
		logger.Log.Warningf("Failed to fetch updated Stored %s data from the DB: %v", e.cfg.RequestType, err)
		if _, ok := err.(net.Error); ok {
			e.recordError(metrics.StoredDataErrorNetwork)
		} else {
			e.recordError(metrics.StoredDataErrorUndefined)
	...
	if err := e.sendEvents(rows); err != nil {
		logger.Log.Warningf("Failed to load updated Stored %s data from the DB: %v", e.cfg.RequestType, err)
		e.recordError(metrics.StoredDataErrorUndefined)
		return err
	}
...
func (e *DatabaseEventProducer) sendEvents(rows *sql.Rows) (err error) {
	...
	for rows.Next() {
		...
		default:
			logger.Log.Warningf("Stored Data with id=%s has invalid type: %s. This will be ignored.", id, dataType)
		}
	}
