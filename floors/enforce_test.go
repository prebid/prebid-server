...
func TestEnforceFloors(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "Should not enforce floors for deals, ext.prebid.floors.enforcement.floorDeals not provided and floors enabled = true",
			args: args{
				bidRequestWrapper: &openrtb_ext.RequestWrapper{
					BidRequest: &openrtb2.BidRequest{
						ID: "some-request-id",
			...
			want1: []error{errors.New("bid rejected [bid ID: some-bid-11] reason: bid price value 0.5000 USD is less than bidFloor value 20.0100 USD for impression id some-impression-id-1 bidder appnexus")},
		},
		{
			name: "Should not enforce floors for deals, ext.prebid.floors.enforcement.floorDeals=false and floors enabled = true",
			args: args{
				bidRequestWrapper: &openrtb_ext.RequestWrapper{
					BidRequest: &openrtb2.BidRequest{
						ID: "some-request-id",
		...
		{
			name: "Should not enforce floors for deals, ext.prebid.floors.enforcement.floorDeals=true and EnforceDealFloors = false in account config",
			args: args{
				bidRequestWrapper: &openrtb_ext.RequestWrapper{
					BidRequest: &openrtb2.BidRequest{
						ID: "some-request-id",
				...
				conversions:    convert{},
				priceFloorsCfg: config.AccountPriceFloors{Enabled: true, EnforceFloorRate: 100, EnforceDealFloors: false},
			},
			want: map[openrtb_ext.BidderName]*entities.PbsOrtbSeatBid{
				"pubmatic": {
					Bids: []*entities.PbsOrtbBid{
		...
		},
		{
			...
			want1: []error{errors.New("bid rejected [bid ID: some-bid-11] reason: bid price value 4.5000 USD is less than bidFloor value 5.0100 USD for impression id some-impression-id-1 bidder appnexus")},
		},
		{
			...
			args: args{
				...
				conversions:    convert{},
				priceFloorsCfg: config.AccountPriceFloors{Enabled: false, EnforceFloorRate: 100, EnforceDealFloors: true},
			},
			want: map[openrtb_ext.BidderName]*entities.PbsOrtbSeatBid{
				"pubmatic": {
					Bids: []*entities.PbsOrtbBid{
		...
		},
		{
			...
			want1: []error{errors.New("bid rejected [bid ID: some-bid-1] reason: bid price value 1.2000 USD is less than bidFloor value 5.0100 USD for impression id some-impression-id-1 bidder pubmatic"),
				errors.New("bid rejected [bid ID: some-bid-11] reason: bid price value 4.5000 USD is less than bidFloor value 5.0100 USD for impression id some-impression-id-1 bidder appnexus")},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
