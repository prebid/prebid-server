...
func TestCookieSyncHandle(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			expectedBody:       `JSON parsing failed: readObjectStart: expect { or n, but found m, error found in #1 byte of ...|malformed|..., bigger context ...|malformed|...` + "\n",
			setMetricsExpectations: func(m *metrics.MetricsEngineMock) {
			...
			setAnalyticsExpectations: func(a *MockAnalyticsRunner) {
				expected := analytics.CookieSyncObject{
					Status:       400,
					Errors:       []error{errors.New("JSON parsing failed: readObjectStart: expect { or n, but found m, error found in #1 byte of ...|malformed|..., bigger context ...|malformed|...")},
					BidderStatus: []*analytics.CookieSyncBidder{},
				}
				a.On("LogCookieSyncObject", &expected).Once()
			},
...
func TestCookieSyncParseRequest(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			givenGDPRConfig:  config.GDPR{Enabled: true, DefaultValue: "0"},
			givenCCPAEnabled: true,
			expectedError:    "JSON parsing failed: readObjectStart: expect { or n, but found m, error found in #1 byte of ...|malformed|..., bigger context ...|malformed|...",
		},
		{
