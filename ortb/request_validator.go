...
func (srv *standardRequestValidator) ValidateImp(imp *openrtb_ext.ImpWrapper, cfg ValidationConfig, index int, aliases map[string]string, hasStoredAuctionResponses bool, storedBidResponses stored_responses.ImpBidderStoredResp) []error {
	if imp.ID == "" {
		return []error{&errortypes.BadInput{Message: fmt.Sprintf("request.imp[%d] missing required field: \"id\"", index)}}
	}

	if len(imp.Metric) != 0 {
		return []error{&errortypes.BadInput{Message: fmt.Sprintf("request.imp[%d].metric is not yet supported by prebid-server. Support may be added in the future", index)}}
	}

	if imp.Banner == nil && imp.Video == nil && imp.Audio == nil && imp.Native == nil {
		return []error{&errortypes.BadInput{Message: fmt.Sprintf("request.imp[%d] must contain at least one of \"banner\", \"video\", \"audio\", or \"native\"", index)}}
	}
...
func (srv *standardRequestValidator) validateImpExt(imp *openrtb_ext.ImpWrapper, cfg ValidationConfig, aliases map[string]string, impIndex int, hasStoredAuctionResponses bool, storedBidResp stored_responses.ImpBidderStoredResp) []error {
	if len(imp.Ext) == 0 {
		return []error{&errortypes.BadInput{Message: fmt.Sprintf("request.imp[%d].ext is required", impIndex)}}
	}
	...
	if hasStoredAuctionResponses && prebid.StoredAuctionResponse == nil {
		return []error{&errortypes.BadInput{Message: fmt.Sprintf("request validation failed. The StoredAuctionResponse.ID field must be completely present with, or completely absent from, all impressions in request. No StoredAuctionResponse data found for request.imp[%d].ext.prebid \n", impIndex)}}
	}
		...
		if coreBidderNormalized, isValid := srv.bidderMap[coreBidder.String()]; isValid {
			if !cfg.SkipBidderParams {
				if err := srv.paramsValidator.Validate(coreBidderNormalized, val); err != nil {
					return []error{&errortypes.BadInput{Message: fmt.Sprintf("request.imp[%d].ext.prebid.bidder.%s failed validation.\n%v", impIndex, bidder, err)}}
				}
			}
		} else {
			if msg, isDisabled := srv.disabledBidders[bidder]; isDisabled {
				...
				return []error{&errortypes.BadInput{Message: fmt.Sprintf("request.imp[%d].ext.prebid.bidder contains unknown bidder: %s. Did you forget an alias in request.ext.prebid.aliases?", impIndex, bidder)}}
			}
		}
	}

	if len(prebid.Bidder) == 0 {
		errL = append(errL, &errortypes.BadInput{Message: fmt.Sprintf("request.imp[%d].ext.prebid.bidder must contain at least one bidder", impIndex)})
		return errL
	}
	...
	if bidResponses, ok := storedBidResp[impId]; ok {
		...
		for bidderName := range bidResponses {
			if _, bidderNameOk := openrtb_ext.NormalizeBidderName(bidderName); !bidderNameOk {
				return &errortypes.BadInput{Message: fmt.Sprintf(`unrecognized bidder "%v"`, bidderName)}
			}
...
func generateStoredBidResponseValidationError(impID string) error {
	return &errortypes.BadInput{Message: fmt.Sprintf("request validation failed. Stored bid responses are specified for imp %s. Bidders specified in imp.ext should match with bidders specified in imp.ext.prebid.storedbidresponse", impID)}
}
