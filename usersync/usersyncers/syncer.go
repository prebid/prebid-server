...
func NewSyncerMap(cfg *config.Configuration) map[openrtb_ext.BidderName]usersync.Usersyncer {
	return map[openrtb_ext.BidderName]usersync.Usersyncer{
		...
		openrtb_ext.BidderTtx:          NewTtxSyncer(cfg.ExternalURL, cfg.Adapters[string(openrtb_ext.BidderTtx)].UserSyncURL, cfg.Adapters[string(openrtb_ext.BidderTtx)].PartnerId),
	}
}
