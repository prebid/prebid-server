...
func NewSyncerMap(cfg *config.Configuration) map[openrtb_ext.BidderName]usersync.Usersyncer {
	return map[openrtb_ext.BidderName]usersync.Usersyncer{
		...
		openrtb_ext.Bidder33Across:     New33AcrossSyncer(cfg.ExternalURL, cfg.Adapters[string(openrtb_ext.Bidder33Across)].UserSyncURL, cfg.Adapters[string(openrtb_ext.Bidder33Across)].PartnerId),
	}
}
