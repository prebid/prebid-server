...
func newJsonDirectoryServer(schemaDirectory string, validator openrtb_ext.BidderParamValidator, aliases map[openrtb_ext.BidderName]openrtb_ext.BidderName) httprouter.Handle {
	...
	if err != nil {
		logger.Log.Fatalf("Failed to read directory %s: %v", schemaDirectory, err)
	}
	...
	for _, file := range files {
		...
		if !isValid {
			logger.Log.Fatalf("Schema exists for an unknown bidder: %s", bidder)
		}
	...
	if err != nil {
		logger.Log.Fatalf("Failed to marshal bidder param JSON-schema: %v", err)
	}
...
func New(cfg *config.Configuration, rateConvertor *currency.RateConverter) (r *Router, err error) {
	...
	if readCertErr != nil {
		logger.Log.Infof("Could not read certificates file: %s \n", readCertErr.Error())
	}
	...
	if err != nil {
		logger.Log.Fatalf("Failed to init hook modules: %v", err)
	}
	...
	if err != nil {
		logger.Log.Fatalf("Failed to create the bidder params validator. %v", err)
	}
	...
	if err != nil {
		logger.Log.Fatalf("Failed to create ads cert signer: %v", err)
	}
	...
	if err != nil {
		logger.Log.Fatalf("Failed to create the openrtb2 endpoint handler. %v", err)
	}
	...
	if err != nil {
		logger.Log.Fatalf("Failed to create the amp endpoint handler. %v", err)
	}
	...
	if err != nil {
		logger.Log.Fatalf("Failed to create the video endpoint handler. %v", err)
	}
...
func (r *Router) Shutdown() {
	logger.Log.Info("[PBS Router] shutting down")
	for _, shutdown := range r.shutdowns {
		shutdown()
	}
	logger.Log.Info("[PBS Router] shut down")
}
		...
		for _, endpoint := range info.Syncer.Supports {
			...
			case "iframe":
				if info.Syncer.IFrame == nil {
					logger.Log.Warningf("bidder %s supports iframe user sync, but doesn't have a default and must be configured by the host", name)
				}
			case "redirect":
				if info.Syncer.Redirect == nil {
					logger.Log.Warningf("bidder %s supports redirect user sync, but doesn't have a default and must be configured by the host", name)
				}
			default:
...
func readDefaultRequestFromFile(defReqConfig config.DefReqConfig) []byte {
	...
	if err != nil {
		logger.Log.Fatalf("error reading default request from file %s: %v", defReqConfig.FileSystem.FileName, err)
		return []byte{}
	}

	// validate json is valid
	if err := jsonutil.UnmarshalValid(defaultRequestJSON, &openrtb2model.BidRequest{}); err != nil {
		logger.Log.Fatalf("error parsing default request from file %s: %v", defReqConfig.FileSystem.FileName, err)
		return []byte{}
	}
