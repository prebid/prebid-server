...
func (fpd *fpdAvailable) Call(payload *hookstage.ProcessedAuctionRequestPayload) (string, error) {
	if payload == nil || payload.GetBidderRequestPayload() == nil || payload.GetBidderRequestPayload().BidRequest == nil {
		return "false", nil
	}

	if found, _ := hasSiteContentDataOrSiteExtData(payload.GetBidderRequestPayload()); found == "true" {
		return "true", nil
	}

	if found, _ := hasAppContentDataOrAppExtData(payload.GetBidderRequestPayload()); found == "true" {
		return "true", nil
	}
...
func (sid *gppSidIn) Call(payload *hookstage.ProcessedAuctionRequestPayload) (string, error) {
	...
	if !hasGPPSIDs(payload.GetBidderRequestPayload()) {
		return "false", nil
	}

	for i := range payload.GetBidderRequestPayload().Regs.GPPSID {
		if _, found := sid.GppSids[payload.GetBidderRequestPayload().Regs.GPPSID[i]]; found {
			return "true", nil
		}
	}
