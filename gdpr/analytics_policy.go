...
func NewAnalyticsPolicyBuilder(cfg config.GDPR, vendorListFetcher VendorListFetcher) PrivacyPolicyBuilder {
	return func(tcf2Cfg TCF2ConfigReader, gdprSignal Signal, consent string) PrivacyPolicy {
		...
		return NewAnalyticsPolicy(cfg, tcf2Cfg, vendorListFetcher, purposeEnforcerBuilder, gdprSignal, consent)
	}
}

// NewAnalyticsPolicy returns a request-specific analytics privacy policy
func NewAnalyticsPolicy(cfg config.GDPR, tcf2Cfg TCF2ConfigReader, fetcher VendorListFetcher, purposeEnforcerBuilder PurposeEnforcerBuilder, gdprSignal Signal, consent string) PrivacyPolicy {
	if !cfg.Enabled {
...
func (ap *analyticsPolicy) Allow(ctx context.Context, name string, vendorID uint16) (bool, error) {
