...
func TestAllowPersonalInfo(t *testing.T) {
	...
	for _, tt := range tests {
		...
		allowPI, _, _, err := perms.PersonalInfoAllowed(context.Background(), tt.bidderName, tt.publisherID, tt.gdpr, tt.consent, tt.basicEnforce)
...
func TestAllowPersonalInfoTCF2(t *testing.T) {
	...
	for _, td := range testDefs {
		allowPI, allowGeo, allowID, err := perms.PersonalInfoAllowed(context.Background(), td.bidder, "", SignalYes, td.consent, td.basicEnforce)
