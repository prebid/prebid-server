...
RUN go build -mod=vendor -ldflags "-X github.com/cafemedia/prebid-server-go-fork/v2/version.Ver=`git describe --tags | sed 's/^v//'` -X github.com/cafemedia/prebid-server-go-fork/v2/version.Rev=`git rev-parse HEAD`" .

FROM ubuntu:20.04 AS release
LABEL maintainer="raptive" 
