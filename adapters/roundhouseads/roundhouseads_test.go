...
func TestEndpointTemplateMalformed(t *testing.T) {
	_, buildErr := Builder(openrtb_ext.BidderRoundhouseads, config.Adapter{
		Endpoint: "{{Malformed}}"},
...
func TestCorrectConfig(t *testing.T) {
	bidder, buildErr := Builder(openrtb_ext.BidderRoundhouseads, config.Adapter{
		Endpoint:         `http://it.doesnt.matter/bid`,
...
func TestEmptyConfig(t *testing.T) {
	bidder, buildErr := Builder(openrtb_ext.BidderRoundhouseads, config.Adapter{
		Endpoint:         `https://roundhouseads.com/openrtb2`,
		ExtraAdapterInfo: `roundhouseads`,
	},
		config.Server{ExternalUrl: "http://hosturl.com", GvlID: 1, DataCenter: "2"})
...
func TestJsonSamples(t *testing.T) {
	bidder, err := Builder(
		openrtb_ext.BidderRoundhouseads,
		config.Adapter{Endpoint: "https://roundhouseads.com/openrtb2", ExtraAdapterInfo: "roundhouseads"},
		config.Server{ExternalUrl: "http://hosturl.com", GvlID: 1, DataCenter: "1"},
	)
	assert.Nil(t, err)
	adapterstest.RunJSONBidderTest(t, "roundhouseadstest", bidder)
}
...
func TestRoundhouseadsAdapter_MakeRequests(t *testing.T) {
	type fields struct {
		URI string
	}
	type args struct {
...
func TestRoundhouseadsAdapter_MakeBids(t *testing.T) {
	type fields struct {
		URI string
	}
	type args struct {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				response: &adapters.ResponseData{
					...
					Body:       []byte(`{"id": "test-request-id", "seatbid":[{"seat": "958", "bid":[{"mtype": 1, "id": "7706636740145184841", "impid": "test-imp-id", "price": 0.500000, "adid": "29681110", "adm": "some-test-ad", "adomain":["roundhouseads.com"], "crid": "29681110", "h": 250, "w": 300, "dealid": "testdeal", "ext":{"dspid": 6, "deal_channel": 1, "prebiddealpriority": 1}}]}], "bidid": "5778926625248726496", "cur": "USD"}`),
				},
			},
			wantErr: nil,
			wantResp: &adapters.BidderResponse{
				Bids: []*adapters.TypedBid{
					{
						Bid: &openrtb2.Bid{
							...
							AdID:    "29681110",
							AdM:     "some-test-ad",
							ADomain: []string{"roundhouseads.com"},
							CrID:    "29681110",
							H:       250,
							W:       300,
							DealID:  "testdeal",
		...
		},
		{
			...
			args: args{
				response: &adapters.ResponseData{
					StatusCode: http.StatusOK,
					Body:       []byte(`{"id": "test-request-id", "seatbid":[{"seat": "958", "bid":[{"mtype": 2, "id": "7706636740145184841", "impid": "test-imp-id", "price": 0.500000, "adid": "29681110", "adm": "some-test-ad", "adomain":["roundhouseads.com"], "crid": "29681110", "h": 250, "w": 300, "dealid": "testdeal", "ext":{"dspid": 6, "deal_channel": 1, "prebiddealpriority": -1}}]}], "bidid": "5778926625248726496", "cur": "USD"}`),
				},
			},
			wantErr: nil,
			wantResp: &adapters.BidderResponse{
