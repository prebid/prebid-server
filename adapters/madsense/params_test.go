...
func TestValidParams(t *testing.T) {
	...
	for _, validParam := range validParams {
		if err := validator.Validate(openrtb_ext.BidderMadSense, json.RawMessage(validParam)); err != nil {
			t.Errorf("Schema rejected madsense params: %s", validParam)
		}
	}
}
...
func TestInvalidParams(t *testing.T) {
	...
	for _, invalidParam := range invalidParams {
		if err := validator.Validate(openrtb_ext.BidderMadSense, json.RawMessage(invalidParam)); err == nil {
