...
type reqBodyExt struct {
	PlayVid360BidderExt reqBodyExtBidder `json:"bidder"`
}
...
func (a *adapter) MakeRequests(request *openrtb2.BidRequest, reqInfo *adapters.ExtraRequestInfo) ([]*adapters.RequestData, []error) {
	...
	for _, imp := range request.Imp {
		...
		var playVid360Ext openrtb_ext.ImpExt360PlayVid

		if err := jsonutil.Unmarshal(imp.Ext, &bidderExt); err != nil {
			errs = append(errs, err)
			continue
		}
		if err := jsonutil.Unmarshal(bidderExt.Bidder, &playVid360Ext); err != nil {
			errs = append(errs, err)
			continue
		}
		...
		if playVid360Ext.PlacementID != "" {
			...
			impExt.PlayVid360BidderExt.Type = "publisher"
		} else if playVid360Ext.EndpointID != "" {
			impExt.PlayVid360BidderExt.EndpointID = playVid360Ext.EndpointID
			impExt.PlayVid360BidderExt.Type = "network"
		}
