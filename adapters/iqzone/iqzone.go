...
type IQZoneAdapter struct {
	EndpointTemplate template.Template
}

//MakeRequests prepares request information for prebid-server core
func (adapter *IQZoneAdapter) MakeRequests(request *openrtb.BidRequest, reqInfo *adapters.ExtraRequestInfo) ([]*adapters.RequestData, []error) {
...
func getImpressionsInfo(imps []openrtb.Imp) (map[openrtb_ext.ExtImpIQZone][]openrtb.Imp, []openrtb.Imp, []error) {
	errors := make([]error, 0, len(imps))
	resImps := make([]openrtb.Imp, 0, len(imps))
	res := make(map[openrtb_ext.ExtImpIQZone][]openrtb.Imp)
	...
	for _, imp := range imps {
		...
		//dispatchImpressions
		//Group impressions by IQZone-specific parameters `pubid
...
func validateImpression(impExt *openrtb_ext.ExtImpIQZone) error {
...
//Alter impression info to comply with IQZone platform requirements
func compatImpression(imp *openrtb.Imp) error {
	imp.Ext = nil //do not forward ext to IQZone platform
	if imp.Banner != nil {
...
func compatBannerImpression(imp *openrtb.Imp) error {
	...
	banner := &bannerCopy
	//As banner.w/h are required fields for IQZone platform - take the first format entry
...
func getImpressionExt(imp *openrtb.Imp) (*openrtb_ext.ExtImpIQZone, error) {
	...
	var IQZoneExt openrtb_ext.ExtImpIQZone
	if err := json.Unmarshal(bidderExt.Bidder, &IQZoneExt); err != nil {
		return nil, &errortypes.BadInput{
			Message: err.Error(),
		}
	}
	return &IQZoneExt, nil
}

func (adapter *IQZoneAdapter) buildAdapterRequest(prebidBidRequest *openrtb.BidRequest, params *openrtb_ext.ExtImpIQZone, imps []openrtb.Imp) (*adapters.RequestData, error) {
...
func createBidRequest(prebidBidRequest *openrtb.BidRequest, params *openrtb_ext.ExtImpIQZone, imps []openrtb.Imp) *openrtb.BidRequest {
...
func (adapter *IQZoneAdapter) buildEndpointURL(params *openrtb_ext.ExtImpIQZone) (string, error) {
...
// NewIQZoneAdapter to be called in prebid-server core to create IQZone adapter instance
func NewIQZoneBidder(endpointTemplate string) adapters.Bidder {
	template, err := template.New("endpointTemplate").Parse(endpointTemplate)
	if err != nil {
		return nil
	}
	return &IQZoneAdapter{EndpointTemplate: *template}
}
