...
func (a *LiftoffAdapter) MakeRequests(request *openrtb.BidRequest, reqInfo *adapters.ExtraRequestInfo) ([]*adapters.RequestData, []error) {
	...
	for i := 0; i < numRequests; i++ {
		...
		if thisImp.Video != nil {
			...
			sanitizedCompanions := []openrtb.Banner{}
			for _, companion := range videoCopy.CompanionAd {
				companion.Ext = nil
				sanitizedCompanions = append(sanitizedCompanions, companion)
			}
			videoCopy.CompanionAd = sanitizedCompanions
