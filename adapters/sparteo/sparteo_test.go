...
func TestMakeRequests_AppBundle_WithMissingDomain_WarnsAndUsesUnknown(t *testing.T) {
	endpoint := "https://bid-test.sparteo.com/s2s-auction?network_id={{.NetworkId}}{{if .AppDomain}}&app_domain={{.AppDomain}}{{end}}{{if .Bundle}}&bundle={{.Bundle}}{{end}}"
	...
	assert.Equal(t,
		"https://bid-test.sparteo.com/s2s-auction?network_id=networkID&app_domain=unknown&bundle=com.sparteo.app",
		reqs[0].Uri,
	)
...
// TestMakeRequests_AppEmptyDomain_WarnsAndUnknown ensures App with empty domain yields app_domain=unknown and one domain warning.
func TestMakeRequests_AppEmptyDomain_WarnsAndUnknown(t *testing.T) {
	endpoint := "https://host/s2s?network_id={{.NetworkId}}{{if .AppDomain}}&app_domain={{.AppDomain}}{{end}}{{if .Bundle}}&bundle={{.Bundle}}{{end}}"
	bidder, err := Builder(openrtb_ext.BidderSparteo, config.Adapter{Endpoint: endpoint}, config.Server{GvlID: 1028})
	require.NoError(t, err)

	in := &openrtb2.BidRequest{
		App: &openrtb2.App{Domain: "", Bundle: "com.app", Publisher: &openrtb2.Publisher{}},
		Imp: []openrtb2.Imp{{ID: "i1", Ext: json.RawMessage(`{"bidder":{"networkId":"N"}}`)}},
	}
	reqs, errs := bidder.MakeRequests(in, &adapters.ExtraRequestInfo{})
	require.Len(t, reqs, 1)
	require.Len(t, errs, 1)
	assert.Equal(t, "https://host/s2s?network_id=N&app_domain=unknown&bundle=com.app", reqs[0].Uri)
	var badInput *errortypes.BadInput
	require.True(t, errors.As(errs[0], &badInput))
	assert.Contains(t, badInput.Error(), "Domain not found")
}
