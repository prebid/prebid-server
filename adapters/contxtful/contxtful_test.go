...
// TestGlobalPrivacyControlHeader tests that GPC header is correctly set when provided
func TestGlobalPrivacyControlHeader(t *testing.T) {
	bidder, buildErr := Builder(openrtb_ext.BidderContxtful, config.Adapter{
		Endpoint: "https://prebid.receptivity.io/v1/pbs/{{.AccountID}}/bid",
	}, config.Server{})
	...
	testCases := []struct {
		...
		{
			name:           "With GPC header set to 1",
			gpcHeaderValue: "1",
			expectedHeader: "1",
			description:    "GPC header should be set to 1 when provided",
		},
		{
			name:           "With GPC header set to 0",
			gpcHeaderValue: "0",
			expectedHeader: "0",
			description:    "GPC header should be set to 0 when provided",
		},
		{
			name:           "With empty GPC header",
			gpcHeaderValue: "",
			expectedHeader: "",
			description:    "No GPC header should be set when empty",
		},
	}
		...
		"imp": [
			{
				"id": "test-imp",
				"banner": {"format": [{"w": 300, "h": 250}]},
				"ext": {"bidder": {"placementId": "test-placement", "customerId": "GPC123"}}
			}
		]
	}`
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			reqInfo := &adapters.ExtraRequestInfo{
				GlobalPrivacyControlHeader: tc.gpcHeaderValue,
			}

			requests, errs := bidder.MakeRequests(&request, reqInfo)
			...
			gpcHeader := requests[0].Headers.Get("Sec-GPC")
			if tc.expectedHeader == "" {
				if gpcHeader != "" {
					t.Errorf("Expected no Sec-GPC header when GPC value is empty, got %s", gpcHeader)
				}
			} else {
				if gpcHeader != tc.expectedHeader {
					t.Errorf("Expected Sec-GPC header %s, got %s", tc.expectedHeader, gpcHeader)
				}
			}
		})
	}
