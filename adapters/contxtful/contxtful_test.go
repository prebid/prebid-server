...
func TestEventTrackingURLGeneration(t *testing.T) {
	...
	responseJSON := `[{
		...
		"random": 0.742856,
		"bids": [{
			"impid": "test-imp-events",
			"price": 3.50,
			"adm": "<div>Event Tracking Test Ad</div>",
			"w": 728,
			"h": 90,
			"crid": "event-creative-1"
		}]
	}]`
	...
	// Verify NURL (win notice URL) is set
	if bid.Bid.NURL == "" {
		t.Error("NURL should be set for event tracking")
	} else {
		...
		if !contains(bid.Bid.NURL, "domain=example.com") {
			t.Error("NURL should contain domain=example.com parameter")
		}
		if !contains(bid.Bid.NURL, "traceId=trace-abc-123") {
			t.Error("NURL should contain traceId=trace-abc-123 parameter")
		}
		if !contains(bid.Bid.NURL, "adRequestId=req-abc-456") {
			t.Error("NURL should contain adRequestId=req-abc-456 parameter")
		}
		if !contains(bid.Bid.NURL, "random=0.742856") {
			t.Error("NURL should contain random=0.742856 parameter")
		}
		if !contains(bid.Bid.NURL, "impId=test-imp-events") {
			t.Error("NURL should contain impId=test-imp-events parameter")
		}
		if !contains(bid.Bid.NURL, "price=3.50") {
			t.Error("NURL should contain price=3.50 parameter")
		}
		if !contains(bid.Bid.NURL, "w=728") {
			t.Error("NURL should contain w=728 parameter")
		}
		if !contains(bid.Bid.NURL, "h=90") {
			t.Error("NURL should contain h=90 parameter")
		}
	}

	// Verify BURL (billing URL) is set
	if bid.Bid.BURL == "" {
		t.Error("BURL should be set for event tracking")
	} else {
		...
		if !contains(bid.Bid.BURL, "domain=example.com") {
			t.Error("BURL should contain domain=example.com parameter")
		}
		if !contains(bid.Bid.BURL, "traceId=trace-abc-123") {
			t.Error("BURL should contain traceId=trace-abc-123 parameter")
		}
		if !contains(bid.Bid.BURL, "adRequestId=req-abc-456") {
			t.Error("BURL should contain adRequestId=req-abc-456 parameter")
		}
		if !contains(bid.Bid.BURL, "random=0.742856") {
			t.Error("BURL should contain random=0.742856 parameter")
		}
		if !contains(bid.Bid.BURL, "impId=test-imp-events") {
			t.Error("BURL should contain impId=test-imp-events parameter")
		}
		if !contains(bid.Bid.BURL, "price=3.50") {
			t.Error("BURL should contain price=3.50 parameter")
		}
		if !contains(bid.Bid.BURL, "w=728") {
			t.Error("BURL should contain w=728 parameter")
		}
		if !contains(bid.Bid.BURL, "h=90") {
			t.Error("BURL should contain h=90 parameter")
		}
	}
}
...
func TestResponseFormatDetectionPriority(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name: "Relay Format (with bids array)",
			responseBody: `[{
				"traceId": "relay-trace-456-def",
				"random": 0.1234567,
				"bids": [{
					"impid": "test-imp-format",
					"price": 4.25,
					"adm": "<div>Relay Format Ad</div>",
					"w": 300,
					"h": 250,
					"crid": "relay-creative"
				}]
			}]`,
			expectedFormat: "Relay",
			expectedBids:   1,
			expectedError:  false,
			validateContent: func(t *testing.T, bids *adapters.BidderResponse) {
				if len(bids.Bids) != 1 {
				...
				// Should use tracking values from relay response
				nurl := bids.Bids[0].Bid.NURL
				if !contains(nurl, "traceId=relay-trace-456-def") {
					t.Error("Relay format should use traceId=relay-trace-456-def")
				}
				if !contains(nurl, "random=0.123457") {
					t.Error("Relay format should use random=0.123457")
				}
			},
		},
		{
...
func TestTrackingValuePropagation(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name: "Relay Response with Full Tracking",
			responseBody: `[{
				"traceId": "full-relay-trace-abc-123",
				"random": 0.987654321,
				"bids": [{
					"impid": "tracking-imp-1",
					"price": 5.75,
					"adm": "<div>Full Tracking Relay Ad</div>",
					"w": 728,
					"h": 90,
					"crid": "relay-tracking-creative"
				}]
			}]`,
			requestID: "full-tracking-request-123",
			domain:    "tracking.example.com",
			customer:  "TRACK123",
			expectedValues: map[string]string{
				"traceId":     "full-relay-trace-abc-123",
				"random":      "0.987654", // Note: formatted with 6 decimal places
				"adRequestId": "full-tracking-request-123",
				"domain":      "tracking.example.com",
				"a":           "TRACK123",
				"price":       "5.75",
				"w":           "728",
...
func TestEventURLGeneration(t *testing.T) {
	...
	// Test Case 1: Relay response with comprehensive tracking
	relayResponseBody := `[{
		"traceId": "event-trace-789",
		"random": 0.123456789,
		"bids": [{
			"impid": "test-imp-event",
			"price": 1.25,
			"currency": "USD",
			"cpm": 1.25,
			"width": 300,
...
func TestBidExtensionsRegression(t *testing.T) {
	...
	assert.Contains(t, bid.NURL, "t=win", "NURL should contain win tracking parameter")
	assert.Contains(t, bid.BURL, "t=billing", "BURL should contain billing tracking parameter")
...
func TestEventURLGenerationNoDoubleSlash(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			requestURI:         "https://prebid.receptivity.io/v1/prebid/PRODCUST123/bid",
			expectedCustomer:   "PRODCUST123",
			monitoringEndpoint: "https://monitoring.receptivity.io",
		},
		{
			name:               "Development endpoint",
			requestURI:         "https://prebid.receptivity.dev/v1/prebid/DEVCUST456/bid",
			expectedCustomer:   "DEVCUST456",
			monitoringEndpoint: "https://monitoring.receptivity.dev",
		},
		{
			name:               "Monitoring endpoint with trailing slash",
			requestURI:         "https://prebid.receptivity.io/v1/prebid/SLASHTEST789/bid",
			expectedCustomer:   "SLASHTEST789",
			monitoringEndpoint: "https://monitoring.receptivity.io/", // With trailing slash
		},
		{
			name:               "Localhost development",
			requestURI:         "http://localhost:6789/v1/prebid/LOCALTEST999/bid",
			expectedCustomer:   "LOCALTEST999",
			monitoringEndpoint: "http://localhost:6789",
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			originalEndpoint := adapter.monitoringEndpoint
			adapter.monitoringEndpoint = tc.monitoringEndpoint
			defer func() { adapter.monitoringEndpoint = originalEndpoint }()

			responseJSON := `[{
				"traceId": "no-double-slash-test-456",
				"random": 0.123456,
				"bids": [{
					"impid": "test-imp-no-double-slash",
					"price": 3.75,
					"adm": "<div>No Double Slash Test Ad</div>",
					"w": 300,
					"h": 250,
					"crid": "no-double-slash-creative"
				}]
			}]`
...
func TestNURLBURLFromPrebidJSResponse(t *testing.T) {
	...
	// Verify NURL contains the response NURL as 'r' parameter
	expectedNURLParam := "r=" + "https%3A%2F%2Fexample.com%2Fwin%3Fprice%3D%24%7BAUCTION_PRICE%7D"
	if !strings.Contains(bid.NURL, expectedNURLParam) {
		t.Errorf("NURL should contain %s, got %s", expectedNURLParam, bid.NURL)
	}

	// Verify BURL contains the response BURL as 'r' parameter
	expectedBURLParam := "r=" + "https%3A%2F%2Fexample.com%2Fbilling%3Fprice%3D%24%7BAUCTION_PRICE%7D"
	if !strings.Contains(bid.BURL, expectedBURLParam) {
		t.Errorf("BURL should contain %s, got %s", expectedBURLParam, bid.BURL)
	}
...
func TestNURLBURLFromContxtfulRelayResponse(t *testing.T) {
	...
	// Mock response with ContxtfulRelay format including NURL/BURL
	responseBody := `[{
		"traceId": "trace-456",
		"random": 0.654321,
		"bids": [{
	...
	// Verify NURL contains the response NURL as 'r' parameter
	expectedNURLParam := "r=" + "https%3A%2F%2Frelay.example.com%2Fnotify%3Fwin%3D1"
	if !strings.Contains(bid.NURL, expectedNURLParam) {
		t.Errorf("NURL should contain %s, got %s", expectedNURLParam, bid.NURL)
	}

	// Verify BURL contains the response BURL as 'r' parameter
	expectedBURLParam := "r=" + "https%3A%2F%2Frelay.example.com%2Fbill%3Fevent%3D1"
	if !strings.Contains(bid.BURL, expectedBURLParam) {
		t.Errorf("BURL should contain %s, got %s", expectedBURLParam, bid.BURL)
	}
...
func TestNURLBURLWithoutResponseURLs(t *testing.T) {
	...
	// Verify URLs don't contain 'r' parameter
	if strings.Contains(bid.NURL, "&r=") {
		t.Error("NURL should not contain r parameter when no response NURL is provided")
	}
	if strings.Contains(bid.BURL, "&r=") {
		t.Error("BURL should not contain r parameter when no response BURL is provided")
	}
...
func TestNURLBURLURLEncoding(t *testing.T) {
	...
	// Verify the complex URL is properly encoded
	expectedNURLParam := "r=" + "https%3A%2F%2Fexample.com%2Fwin%3Fprice%3D%24%7BAUCTION_PRICE%7D%26data%3D%7B%22test%22%3A%22value%22%7D%26special%3Da%2Bb+c%26hash%23fragment"
	if !strings.Contains(bid.NURL, expectedNURLParam) {
		t.Errorf("NURL should contain properly encoded URL parameter %s, got %s", expectedNURLParam, bid.NURL)
	}

	expectedBURLParam := "r=" + "https%3A%2F%2Fexample.com%2Fbilling%3Fprice%3D%24%7BAUCTION_PRICE%7D%26data%3D%7B%22test%22%3A%22value%22%7D%26special%3Da%2Bb+c%26hash%23fragment"
	if !strings.Contains(bid.BURL, expectedBURLParam) {
		t.Errorf("BURL should contain properly encoded URL parameter %s, got %s", expectedBURLParam, bid.BURL)
	}
}
...
func TestNURLBURLPartialResponse(t *testing.T) {
	...
	// Verify BURL does not contain 'r' parameter (since no BURL in response)
	if strings.Contains(bid.BURL, "&r=") {
		t.Error("BURL should not contain r parameter when no response BURL is provided")
	}
}
