...
func (a *LifestreetAdapter) Call(ctx context.Context, req *pbs.PBSRequest, bidder *pbs.PBSBidder) (pbs.PBSBidSlice, error) {
	...
	for i, unit := range bidder.AdUnits {
		lsReq := makeOpenRTBGeneric(req, bidder, a.FamilyName())

		// only grab this ad unit
		lsReq.Imp = lsReq.Imp[i : i+1]
		...
		lsReq.Imp[0].Banner.Format = nil // lifestreet doesn't support format object
		lsReq.Imp[0].TagID = params.SlotTag

		err = json.NewEncoder(&requests[i]).Encode(lsReq)
		if err != nil {
			return nil, err
		}
	}
