...
func TestJsonSamples(t *testing.T) {
	bidder, buildErr := Builder(openrtb_ext.BidderMediafuse, config.Adapter{
	...
	adapterstest.RunJSONBidderTest(t, "mediafusetest", bidder)
}
...
func TestVideoSamples(t *testing.T) {
	...
	adapterstest.RunJSONBidderTest(t, "mediafuseplatformtest", bidder)
}
...
func TestVideoSinglePod(t *testing.T) {
	...
	var reqDataExt *mediafuseReqExt
	error = json.Unmarshal(reqData.Ext, &reqDataExt)
	assert.NoError(t, error, "Response ext unmarshalling error should be nil")

	regMatch, matchErr := regexp.Match(`^[0-9]+$`, []byte(reqDataExt.Mediafuse.AdPodId))
	assert.NoError(t, matchErr, "Regex match error should be nil")
	assert.True(t, regMatch, "AdPod id doesn't present in Mediafuse extension or has incorrect format")
}
...
func TestVideoSinglePodManyImps(t *testing.T) {
	...
	var reqDataExt1 *mediafuseReqExt
	error = json.Unmarshal(reqData1.Ext, &reqDataExt1)
	assert.NoError(t, error, "Response ext unmarshalling error should be nil")

	assert.Equal(t, "", reqDataExt1.Mediafuse.AdPodId, "AdPod id should not be present in first request")

	var reqData2 *openrtb2.BidRequest
	error = json.Unmarshal(res[1].Body, &reqData2)
	assert.NoError(t, error, "Response body unmarshalling error should be nil")

	var reqDataExt2 *mediafuseReqExt
	error = json.Unmarshal(reqData2.Ext, &reqDataExt2)
	assert.NoError(t, error, "Response ext unmarshalling error should be nil")

	assert.Equal(t, "", reqDataExt2.Mediafuse.AdPodId, "AdPod id should not be present in second request")
}
...
func TestVideoTwoPods(t *testing.T) {
	...
	adPodId1 := reqDataExt1.Mediafuse.AdPodId
	...
	adPodId2 := reqDataExt2.Mediafuse.AdPodId
...
func TestVideoTwoPodsManyImps(t *testing.T) {
	...
	var reqDataExt3 *mediafuseReqExt
	error = json.Unmarshal(reqData3.Ext, &reqDataExt3)
	assert.NoError(t, error, "Response ext unmarshalling error should be nil")

	adPodId1 := reqDataExt1.Mediafuse.AdPodId
	adPodId2 := reqDataExt2.Mediafuse.AdPodId
	adPodId3 := reqDataExt3.Mediafuse.AdPodId
