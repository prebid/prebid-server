...
// This file actually intends to test static/bidder-params/mediafuse.json
//
// These also validate the format of the external API: request.imp[i].ext.mediafuse

// TestValidParams makes sure that the mediafuse schema accepts all imp.ext fields which we intend to support.
func TestValidParams(t *testing.T) {
	...
	for _, validParam := range validParams {
		if err := validator.Validate(openrtb_ext.BidderMediafuse, json.RawMessage(validParam)); err != nil {
			t.Errorf("Schema rejected mediafuse params: %s", validParam)
		}
	}
}

// TestInvalidParams makes sure that the mediafuse schema rejects all the imp.ext fields we don't support.
func TestInvalidParams(t *testing.T) {
	...
	for _, invalidParam := range invalidParams {
		if err := validator.Validate(openrtb_ext.BidderMediafuse, json.RawMessage(invalidParam)); err == nil {
