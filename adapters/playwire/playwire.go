...
type PlaywireAdapter struct {
	endpoint string
}

type PlaywireBid struct {
	*openrtb2.Bid
	AdmNative   json.RawMessage     `json:"adm_native,omitempty"`
	ContentType openrtb_ext.BidType `json:"content_type"`
}

type PlaywireSeatBid struct {
	*openrtb2.SeatBid
	Bid []PlaywireBid `json:"bid"`
}

type PlaywireResponse struct {
	*openrtb2.BidResponse
	SeatBid []PlaywireSeatBid `json:"seatbid,omitempty"`
}

type PlaywireBidExt struct {
	Bidder ExtBidder `json:"bidder"`
}

type ExtBidder struct {
	Playwire ExtBidderPlaywire `json:"playwire"`
}

type ExtBidderPlaywire struct {
...
func processImp(imp *openrtb2.Imp) error {
	// get the playwire extension
	var ext adapters.ExtImpBidder
	var playwireExt openrtb_ext.ExtImpPlaywire
	if err := json.Unmarshal(imp.Ext, &ext); err != nil {
		return err
	}
	if err := json.Unmarshal(ext.Bidder, &playwireExt); err != nil {
		return err
	}

	if playwireExt.Uid == 0 {
...
func fixNative(req json.RawMessage) (json.RawMessage, error) {
	var playwireReq map[string]interface{}
	var parsedRequest map[string]interface{}

	if err := json.Unmarshal(req, &playwireReq); err != nil {
		return req, nil
	}
	if imps, exists := maputil.ReadEmbeddedSlice(playwireReq, "imp"); exists {
		for _, imp := range imps {
			if playwireImp, ok := imp.(map[string]interface{}); ok {
				native, hasNative := maputil.ReadEmbeddedMap(playwireImp, "native")
				if hasNative {
	...
	return json.Marshal(playwireReq)
}

// MakeRequests makes the HTTP requests which should be made to fetch bids.
func (a *PlaywireAdapter) MakeRequests(request *openrtb2.BidRequest, reqInfo *adapters.ExtraRequestInfo) ([]*adapters.RequestData, []error) {
	...
	if len(validImps) == 0 {
		err := &errortypes.BadInput{
			Message: "No valid impressions for playwire",
		}
		errors = append(errors, err)
		return nil, errors
	}
...
func (a *PlaywireAdapter) MakeBids(internalRequest *openrtb2.BidRequest, externalRequest *adapters.RequestData, response *adapters.ResponseData) (*adapters.BidderResponse, []error) {
...
// Builder builds a new instance of the Playwire adapter for the given bidder with the given config.
func Builder(bidderName openrtb_ext.BidderName, config config.Adapter, server config.Server) (adapters.Bidder, error) {
	bidder := &PlaywireAdapter{
...
func getBidMeta(ext json.RawMessage) (*openrtb_ext.ExtBidPrebidMeta, error) {
	var bidExt PlaywireBidExt

	if err := json.Unmarshal(ext, &bidExt); err != nil {
		return nil, err
	}
	var bidMeta *openrtb_ext.ExtBidPrebidMeta
	if bidExt.Bidder.Playwire.DemandSource != "" {
		bidMeta = &openrtb_ext.ExtBidPrebidMeta{
			NetworkName: bidExt.Bidder.Playwire.DemandSource,
		}
	}
	return bidMeta, nil
}

func getMediaTypeForImp(impID string, imps []openrtb2.Imp, bidWithType PlaywireBid) (openrtb_ext.BidType, error) {
