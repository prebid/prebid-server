...
func (a *adapter) buildEndpointURL(supplySourceId string) (string, error) {
	if strings.TrimSpace(supplySourceId) == "" {
		return a.defaultEndpoint, nil
	}

	template, err := template.New("endpointTemplate").Parse(a.bidderEndpointTemplate)
	if err != nil {
...
func getExtensionInfo(impressions []openrtb2.Imp) (string, string, error) {
	...
	for _, imp := range impressions {
		...
		if strings.TrimSpace(ttdExt.PublisherId) != "" && strings.TrimSpace(publisherId) == "" {
			publisherId = ttdExt.PublisherId
		}

		if strings.TrimSpace(ttdExt.SupplySourceId) != "" && strings.TrimSpace(supplySourceId) == "" {
			supplySourceId = ttdExt.SupplySourceId
		}

		if strings.TrimSpace(publisherId) != "" && strings.TrimSpace(supplySourceId) != "" {
			break
		}
	}
