...
const MAX_IMPRESSIONS_ANIVIEW = 30

const ae = "ae"

type AniviewAdapter struct {
	URI              string
	authorizationURI string
	bidderName       string
}

type aniviewBidExt struct {
	BidType            *int                `json:"BidType,omitempty"`
	VideoCreativeInfo  *aniviewBidExtVideo `json:"video,omitempty"`
	Marketplace        string              `json:"marketplace,omitempty"`
	PrebidDealPriority int                 `json:"prebiddealpriority,omitempty"`
}

type aniviewWrapperExt struct {
	ProfileID int `json:"profile,omitempty"`
	VersionID int `json:"version,omitempty"`
}

type aniviewBidExtVideo struct {
	Duration *int `json:"duration,omitempty"`
}

type ExtImpBidderAniview struct {
...
type extRequestAdServer struct {
	Wrapper     *aniviewWrapperExt `json:"wrapper,omitempty"`
...
var ANIVIEW_DEFAULT_EXT = `{
            "bidtype": 1
          }`

func (a *AniviewAdapter) MakeRequests(request *openrtb2.BidRequest, reqInfo *adapters.ExtraRequestInfo) ([]*adapters.RequestData, []error) {
	...
	newReqExt, err := extractAniviewExtFromRequest(request)
	if err != nil {
...
// parseImpressionObject parse the imp to get it ready to send to aniview
func parseImpressionObject(imp *openrtb2.Imp, extractWrapperExtFromImp, extractPubIDFromImp bool) (*aniviewWrapperExt, string, error) {
	var wrapExt *aniviewWrapperExt
	var pubID string

	// Aniview supports banner and video impressions.
	if imp.Banner == nil && imp.Video == nil && imp.Native == nil {
		return wrapExt, pubID, fmt.Errorf("invalid MediaType. Aniview only supports Banner, Video and Native. Ignoring ImpID=%s", imp.ID)
	}
	...
	var bidderExt ExtImpBidderAniview
	if err := jsonutil.Unmarshal(imp.Ext, &bidderExt); err != nil {
		return wrapExt, pubID, err
	}

	var aniviewExt openrtb_ext.ExtImpAniview
	if err := jsonutil.Unmarshal(bidderExt.Bidder, &aniviewExt); err != nil {
		return wrapExt, pubID, err
	}

	if extractPubIDFromImp {
		pubID = strings.TrimSpace(aniviewExt.PublisherId)
	}

	// Parse Wrapper Extension only once per request
	if extractWrapperExtFromImp && len(aniviewExt.WrapExt) != 0 {
		err := jsonutil.Unmarshal([]byte(aniviewExt.WrapExt), &wrapExt)
		if err != nil {
			return wrapExt, pubID, fmt.Errorf("Error in Wrapper Parameters = %v  for ImpID = %v WrapperExt = %v", err.Error(), imp.ID, string(aniviewExt.WrapExt))
		}
	}

	if err := validateAdSlot(strings.TrimSpace(aniviewExt.AdSlot), imp); err != nil {
		return wrapExt, pubID, err
	}
	...
	if aniviewExt.Kadfloor != "" {
		bidfloor, err := strconv.ParseFloat(strings.TrimSpace(aniviewExt.Kadfloor), 64)
		if err == nil {
	...
	if aniviewExt.Keywords != nil && len(aniviewExt.Keywords) != 0 {
		addKeywordsToExt(aniviewExt.Keywords, extMap)
	}
	//Give preference to direct values of 'dctr' & 'pmZoneId' params in extension
	if aniviewExt.Dctr != "" {
		extMap[dctrKeyName] = aniviewExt.Dctr
	}
	if aniviewExt.PmZoneID != "" {
		extMap[pmZoneIDKeyName] = aniviewExt.PmZoneID
	}
...
// extractAniviewExtFromRequest parse the req.ext to fetch wrapper and acat params
func extractAniviewExtFromRequest(request *openrtb2.BidRequest) (extRequestAdServer, error) {
...
func getAlternateBidderCodesFromRequestExt(reqExt *openrtb_ext.ExtRequest) []string {
	...
	allowedBidders := []string{"aniview"}
	if reqExt.Prebid.AlternateBidderCodes.Enabled {
		if pmABC, ok := reqExt.Prebid.AlternateBidderCodes.Bidders["aniview"]; ok && pmABC.Enabled {
...
func addKeywordsToExt(keywords []*openrtb_ext.ExtImpAniviewKeyVal, extMap map[string]interface{}) {
...
func (a *AniviewAdapter) MakeBids(internalRequest *openrtb2.BidRequest, externalRequest *adapters.RequestData, response *adapters.ResponseData) (*adapters.BidderResponse, []error) {
	...
	for _, sb := range bidResp.SeatBid {
		for i := 0; i < len(sb.Bid); i++ {
			...
			if bid.Ext == nil {
				bid.Ext = bytes.NewBuffer([]byte(ANIVIEW_DEFAULT_EXT)).Bytes()
			}
...
func getBidType(bidExt *aniviewBidExt) openrtb_ext.BidType {
