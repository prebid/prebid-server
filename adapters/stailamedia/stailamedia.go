...
type stailamediaAdapterOptions struct {
	IabCategories map[string]string `json:"iab_categories"`
}

type stailamediaImpExtStailamedia struct {
...
type stailamediaImpExt struct {
	Stailamedia stailamediaImpExtStailamedia `json:"stailamedia"`
}

type stailamediaBidExtVideo struct {
	Duration int `json:"duration"`
}

type stailamediaBidExtCreative struct {
	Video stailamediaBidExtVideo `json:"video"`
}

type stailamediaBidExtStailamedia struct {
	BidType       int                       `json:"bid_ad_type"`
	BrandId       int                       `json:"brand_id"`
	BrandCategory int                       `json:"brand_category_id"`
	CreativeInfo  stailamediaBidExtCreative `json:"creative_info"`
	DealPriority  int                       `json:"deal_priority"`
}

type stailamediaBidExt struct {
	Stailamedia stailamediaBidExtStailamedia `json:"stailamedia"`
}

type stailamediaReqExtStailamedia struct {
...
// Full request extension including stailamedia extension object
type stailamediaReqExt struct {
	openrtb_ext.ExtRequest
	Stailamedia *stailamediaReqExtStailamedia `json:"stailamedia,omitempty"`
}
...
func (a *adapter) MakeRequests(request *openrtb2.BidRequest, reqInfo *adapters.ExtraRequestInfo) ([]*adapters.RequestData, []error) {
	...
	// Stailamedia openrtb2 endpoint expects imp.displaymanagerver to be populated, but some SDKs will put it in imp.ext.prebid instead
	...
	if len(memberIds) > 0 {
		...
		if len(uniqueIds) > 1 {
			errs = append(errs, fmt.Errorf("All request.imp[i].ext.prebid.bidder.stailamedia.member params must match. Request contained: %v", uniqueIds))
		}
	}
	...
	// Add Stailamedia request level extension
	...
	if reqExt.Stailamedia == nil {
		reqExt.Stailamedia = &stailamediaReqExtStailamedia{}
	}
	includeBrandCategory := reqExt.Prebid.Targeting != nil && reqExt.Prebid.Targeting.IncludeBrandCategory != nil
	if includeBrandCategory {
		reqExt.Stailamedia.BrandCategoryUniqueness = &includeBrandCategory
		reqExt.Stailamedia.IncludeBrandCategory = &includeBrandCategory
	}
	reqExt.Stailamedia.IsAMP = isAMP
	reqExt.Stailamedia.HeaderBiddingSource = a.hbSource + isVIDEO
	...
	if isVIDEO == 1 && *adPodId {
		...
		for _, podImps := range podImps {
			reqExt.Stailamedia.AdPodId = generatePodID()
...
func marshalAndSetRequestExt(request *openrtb2.BidRequest, requestExtension stailamediaReqExt, errs []error) {
	var err error
...
func splitRequests(imps []openrtb2.Imp, request *openrtb2.BidRequest, requestExtension stailamediaReqExt, uri string, errs []error) ([]*adapters.RequestData, []error) {
...
// preprocess mutates the imp to get it ready to send to stailamedia.
//
...
func preprocess(imp *openrtb2.Imp, defaultDisplayManagerVer string) (string, bool, error) {
	...
	var stailamediaExt openrtb_ext.ExtImpStailamedia
	if err := json.Unmarshal(bidderExt.Bidder, &stailamediaExt); err != nil {
		return "", false, err
	}
	...
	if stailamediaExt.PlacementId == 0 && stailamediaExt.LegacyPlacementId != 0 {
		stailamediaExt.PlacementId = stailamediaExt.LegacyPlacementId
	}
	if stailamediaExt.InvCode == "" && stailamediaExt.LegacyInvCode != "" {
		stailamediaExt.InvCode = stailamediaExt.LegacyInvCode
	}
	if stailamediaExt.TrafficSourceCode == "" && stailamediaExt.LegacyTrafficSourceCode != "" {
		stailamediaExt.TrafficSourceCode = stailamediaExt.LegacyTrafficSourceCode
	}

	if stailamediaExt.PlacementId == 0 && (stailamediaExt.InvCode == "" || stailamediaExt.Member == "") {
		return "", false, &errortypes.BadInput{
			Message: "No placement or member+invcode provided",
		}
	}

	if stailamediaExt.InvCode != "" {
		imp.TagID = stailamediaExt.InvCode
	}
	if imp.BidFloor <= 0 && stailamediaExt.Reserve > 0 {
		imp.BidFloor = stailamediaExt.Reserve // This will be broken for non-USD currency.
	}
	if imp.Banner != nil {
		bannerCopy := *imp.Banner
		if stailamediaExt.Position == "above" {
			bannerCopy.Pos = adcom1.PositionAboveFold.Ptr()
		} else if stailamediaExt.Position == "below" {
	...
	impExt := stailamediaImpExt{Stailamedia: stailamediaImpExtStailamedia{
		PlacementID:       stailamediaExt.PlacementId,
		TrafficSourceCode: stailamediaExt.TrafficSourceCode,
		Keywords:          makeKeywordStr(stailamediaExt.Keywords),
		UsePmtRule:        stailamediaExt.UsePmtRule,
		PrivateSizes:      stailamediaExt.PrivateSizes,
	}}
	var err error
	if imp.Ext, err = json.Marshal(&impExt); err != nil {
		return stailamediaExt.Member, stailamediaExt.AdPodId, err
	}

	return stailamediaExt.Member, stailamediaExt.AdPodId, nil
}

func makeKeywordStr(keywords []*openrtb_ext.ExtImpStailamediaKeyVal) string {
			...
			if err := json.Unmarshal(bid.Ext, &bidExt); err != nil {
				...
				if bidType, err := getMediaTypeForBid(&bidExt); err == nil {
					...
					impVideo := &openrtb_ext.ExtBidPrebidVideo{
						Duration: bidExt.Stailamedia.CreativeInfo.Video.Duration,
					}
					...
					bidResponse.Bids = append(bidResponse.Bids, &adapters.TypedBid{
						...
						BidType:      bidType,
						BidVideo:     impVideo,
						DealPriority: bidExt.Stailamedia.DealPriority,
					})
				} else {
					errs = append(errs, err)
				}
...
func getMediaTypeForBid(bid *stailamediaBidExt) (openrtb_ext.BidType, error) {
	switch bid.Stailamedia.BidType {
	case 0:
	...
	default:
		return "", fmt.Errorf("Unrecognized bid_ad_type in response from stailamedia: %d", bid.Stailamedia.BidType)
	}
}

// getIabCategoryForBid maps an stailamedia brand id to an IAB category.
func (a *adapter) getIabCategoryForBid(bid *stailamediaBidExt) (string, error) {
	brandIDString := strconv.Itoa(bid.Stailamedia.BrandCategory)
...
// Builder builds a new instance of the Stailamedia adapter for the given bidder with the given config.
...
func loadCategoryMapFromFileSystem() map[string]string {
	// Load custom options for our adapter (currently just a lookup table to convert stailamedia => iab categories)
	opts, err := os.ReadFile("./static/adapter/stailamedia/opts.json")
	if err == nil {
		var adapterOptions stailamediaAdapterOptions
