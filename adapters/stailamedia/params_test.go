...
// This file actually intends to test static/bidder-params/stailamedia.json
//
// These also validate the format of the external API: request.imp[i].ext.prebid.bidder.stailamedia

// TestValidParams makes sure that the stailamedia schema accepts all imp.ext fields which we intend to support.
func TestValidParams(t *testing.T) {
	...
	for _, validParam := range validParams {
		if err := validator.Validate(openrtb_ext.BidderStailamedia, json.RawMessage(validParam)); err != nil {
			t.Errorf("Schema rejected stailamedia params: %s", validParam)
		}
	}
}

// TestInvalidParams makes sure that the stailamedia schema rejects all the imp.ext fields we don't support.
func TestInvalidParams(t *testing.T) {
	...
	for _, invalidParam := range invalidParams {
		if err := validator.Validate(openrtb_ext.BidderStailamedia, json.RawMessage(invalidParam)); err == nil {
