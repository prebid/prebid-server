...
func (p *PubxaiModule) processAuctionData(ao *analytics.AuctionObject) {
	if ao == nil {
		return
	}
	if ao.RequestWrapper == nil {
		glog.Errorf("RequestWrapper is nil")
		return
	}
	...
	if len(imps) == 0 {
		glog.Errorf("No impressions in request")
		return
	}

	response := ao.Response
	if response == nil {
		glog.Errorf("Response is nil")
		return
	}
	...
	if len(seatBids) == 0 {
		...
		glog.Errorf("No seatbids in response")
		return
	}
	...
	glog.Infof("No of BidResponses: %v", len(BidResponses))
	if len(BidResponses) == 0 {
		glog.Errorf("No matching bids in response")
		return
	}
	p.processBidData(BidResponses, ao)
}

func (p *PubxaiModule) processBidData(bidResponses []map[string]interface{}, ao *analytics.AuctionObject) {
	...
	if err != nil {
		glog.Errorf("Error unmarshalling bidExt: %v", err)
		return
	}
	...
	for _, bidData := range bidResponses {
		...
		if err != nil {
			glog.Errorf("Error unmarshalling impExt: %v", err)
			return
		}
		...
		if p.isWinningBid(bidderName, bid, bidExt) {
			...
			bidObj.IsFloorSkipped = func() bool {
				if val, ok := floorData["isFloorSkipped"].(bool); ok {
					return val
				}
				return false
			}()
			glog.Infof("Enqueue to Winning bid Queue, Current Size: %v", len(p.winBidsQueue.queue))
...
func (bidQueue *BidQueue) isTimeToSend() bool {
	...
	if err != nil {
		glog.Errorf("Error parsing bufferInterval: %v", err)
		return false
	}
	glog.Infof("Time difference: %v, bufferInterval: %v", timeDifference, pDuration)
...
func (bidQueue *BidQueue) flushQueuedData() {
	...
	if resp.StatusCode != http.StatusOK {
		glog.Errorf("Unexpected response status: %s", resp.Status)
	} else {
		glog.Infof("Queued data sent successfully.")
	}
...
func (bidQueue *BidQueue) Enqueue(item Bid) {
	...
	if err != nil {
		glog.Errorf("Error parsing bufferSize: %v", err)
		return
	}
