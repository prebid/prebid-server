...
func (cfg *Configuration) setDerivedDefaults() {
	...
	setDefaultUsersync(cfg.Adapters, openrtb_ext.BidderTriplelift, "https://eb2.3lift.com/uid?gpdr={{.GDPR}}&gdpr_consent={{.GDPRConsent}}")
...
func SetupViper(v *viper.Viper, filename string) {
	...
	v.SetDefault("external_url", "http://localhost:8002")
	v.SetDefault("host", "")
	...
	v.SetDefault("adapters.triplelift.endpoint", "https://tlx.3lift.com/s2s/auction?supplier_id=12")
