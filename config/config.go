...
func (cfg *Configuration) setDerivedDefaults() {
	...
	setDefaultUsersync(cfg.Adapters, openrtb_ext.BidderSmartadserver, "https://diff.smartadserver.com/getuid?gdpr={{.GDPR}}&gdpr_consent={{.GDPRConsent}}&us_privacy={{.USPrivacy}}&url="+url.QueryEscape(externalURL)+"%2Fsetuid%3Fbidder%3Dsmartadserver%26gdpr%3D{{.GDPR}}%26gdpr_consent%3D{{.GDPRConsent}}%26uid%3D%5Bsas_uid%5D%22")
...
func SetupViper(v *viper.Viper, filename string) {
	...
	v.SetDefault("adapters.smartadserver.endpoint", "https://diff.smartadserver.com")
