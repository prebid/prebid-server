...
func (cfg *Configuration) setDerivedDefaults() {
	...
	setDefaultUsersync(cfg.Adapters, openrtb_ext.BidderInvibes, "http://localhost/kweb.website/bid/getLid?gdpr={{.GDPR}}&gdpr_consent={{.GDPRConsent}}&us_privacy={{.USPrivacy}}&redirectUri="+url.QueryEscape(externalURL)+"%2Fsetuid%3Fbidder%3Dinvibes%26gdpr%3D{{.GDPR}}%26gdpr_consent%3D{{.GDPRConsent}}%26uid%3D%24UID")
...
func SetupViper(v *viper.Viper, filename string) {
	...
	v.SetDefault("adapters.invibes.endpoint", "https://adweb.videostepstage.com/bid/ServerBidAdContent")
