...
func TestMigrateConfigDatabaseConnection(t *testing.T) {
	...
	for _, tt := range storedReqestsTests {
		...
		if len(tt.config) > 0 {
			...
			assert.Equal(t, tt.want_connection_host, v.Get("stored_requests.database.connection.host").(string), tt.description)
			assert.Equal(t, tt.want_connection_port, v.Get("stored_requests.database.connection.port").(int), tt.description)
			assert.Equal(t, tt.want_connection_user, v.Get("stored_requests.database.connection.user").(string), tt.description)
			assert.Equal(t, tt.want_connection_password, v.Get("stored_requests.database.connection.password").(string), tt.description)
			assert.Equal(t, tt.want_fetcher_query, v.Get("stored_requests.database.fetcher.query").(string), tt.description)
			assert.Equal(t, tt.want_fetcher_amp_query, v.Get("stored_requests.database.fetcher.amp_query").(string), tt.description)
			assert.Equal(t, tt.want_initialize_caches_timeout_ms, v.Get("stored_requests.database.initialize_caches.timeout_ms").(int), tt.description)
			assert.Equal(t, tt.want_initialize_caches_query, v.Get("stored_requests.database.initialize_caches.query").(string), tt.description)
			assert.Equal(t, tt.want_initialize_caches_amp_query, v.Get("stored_requests.database.initialize_caches.amp_query").(string), tt.description)
			assert.Equal(t, tt.want_poll_for_updates_refresh_rate_seconds, v.Get("stored_requests.database.poll_for_updates.refresh_rate_seconds").(int), tt.description)
			assert.Equal(t, tt.want_poll_for_updates_timeout_ms, v.Get("stored_requests.database.poll_for_updates.timeout_ms").(int), tt.description)
			assert.Equal(t, tt.want_poll_for_updates_query, v.Get("stored_requests.database.poll_for_updates.query").(string), tt.description)
			assert.Equal(t, tt.want_poll_for_updates_amp_query, v.Get("stored_requests.database.poll_for_updates.amp_query").(string), tt.description)
		} else {
